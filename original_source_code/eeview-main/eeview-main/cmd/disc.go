...
func createDeviceCfgYAML(device *eev.DeviceType) error {
	...
	if err != nil {
		return fmt.Errorf("Failed to stat directory %s: %w", outputDir, err)
	}
	if !dirInfo.IsDir() {
		return fmt.Errorf("%s is not a directory", outputDir)
	}
	if dirInfo.Mode().Perm()&0200 == 0 {
		return fmt.Errorf("Directory %s is not writable", outputDir)
	}
	...
	if len(ipParts) != 4 {
		return fmt.Errorf("Invalid IP address format: %s", device.Location.DevIP)
	}
	...
	if err != nil {
		if sysErr, ok := err.(*os.PathError); ok {
			if errno, ok := sysErr.Err.(syscall.Errno); ok {
				return fmt.Errorf("Failed to open YAML file %s (absolute path: %s): %w (errno: %d)", filePath, absPath, err, errno)
			}
		}
		return fmt.Errorf("Failed to open YAML file %s (absolute path: %s): %w", filePath, absPath, err)
	}
	defer file.Close()

	// Write YAML data
	if _, err := file.Write(devYaml); err != nil {
		return fmt.Errorf("Failed to write data to YAML file %s (absolute path: %s): %w", filePath, absPath, err)
	}

	// Ensure data is flushed to disk
	if err := file.Sync(); err != nil {
		return fmt.Errorf("Failed to sync YAML file %s (absolute path: %s): %w", filePath, absPath, err)
	}
